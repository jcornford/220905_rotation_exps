...
from old_lib import update_policies as up
from lib.init_policies import DenseNormalInit
from old_lib.update_policies import SGD, EG
...
class DenseLayerWithLayerNorm(BaseDenseLayer):
    def __init__(self, n_input, n_output, layer_norm=True, nonlinearity=None,
                 weight_init_policy=DenseNormalInit(),
                 update_policy=SGD()):
        """
        n_input:      input dimension
    ...
    def forward(self, x):
        ...
        """
        self.x = x.T
        self.z = torch.mm(self.W, self.x) 

        if self.layer_norm:
            self.mu_z_layer  = self.z.mean(axis=0, keepdim=True) # 1 x batch
            self.std_z_layer = self.z.std(axis=0, keepdim=True) # 1 x batch

            # ne x batch = (1 / 1 x batch) * ( ne x batch - 1 x batch )
            self.z = (1/ self.std_z_layer) * (self.z - self.mu_z_layer) 
