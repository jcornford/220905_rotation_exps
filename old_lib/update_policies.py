...
class Adam():
    ...
    def compute_adam_updates(self,param,m_param,v_param,lr):
        m_param = self.beta1*m_param + (1-self.beta1)*param.grad
        v_param = self.beta2*v_param + (1-self.beta2)*(param.grad)**2
        m_corr = m_param/(1-self.beta1**self.t)
        v_corr = v_param/(1-self.beta2**self.t)
        param -= lr*(m_corr/(torch.sqrt(v_corr)+self.epsilon))
    ...
    def update(self, layer, **kwargs):
        ...
        with torch.no_grad():
            for key, p in layer.named_parameters():
                if p.requires_grad:
                    if not key in self.m.keys():
                        self.m[key] = 0
                        self.v[key] = 0
                    p, self.m[key], self.v[key] = self.compute_adam_updates(p,self.m[key],self.v[key],lr)
                    # p -= p.grad * lr
class EG():
    def __init__(self, **kwargs):
        "required to allow arguments to other mixins"
    ...
    def update(self, layer, **kwargs):
        ...
        with torch.no_grad():
            for key, p in layer.named_parameters():
                if p.requires_grad:
                    p.grad *= torch.sign(p) # removing this only saves a small amount!
...
class EiDense_Adam_UpdatePolicy():
    '''
    This update does SGD (i.e. p.grad * lr) and then clamps Wix, Wex, Wei, g
    to be positive.
    ...
    This should furthest right when composed with mixins for correct MRO.
    '''
    def __init__(self,  beta1=0.9, beta2=0.999, epsilon=1e-8) -> None:
        self.m_dg, self.v_dg = 0, 0
        self.m_db, self.v_db = 0, 0
        self.m_dex, self.v_dex = 0, 0
        self.m_dei, self.v_dei = 0, 0
        self.m_dix, self.v_dix = 0, 0
        self.m_dalpha, self.v_dalpha = 0, 0
        self.beta1 = beta1
        self.beta2 = beta2
        self.epsilon = epsilon
        self.t = 0
    ...
    def update(self, layer, **kwargs):
        ...
        with torch.no_grad():
            if hasattr(layer, 'g'):
                # layer.g -= layer.g.grad *lr
                layer.g,self.m_dg,self.v_dg = self.compute_adam_updates(param=layer.g,
                                                    m_param=self.m_dg,
                                                    v_param=self.v_dg,
                                                    lr=lr)
            if layer.b.requires_grad:
                # layer.b -= layer.b.grad *lr
                layer.b,self.m_db,self.v_db = self.compute_adam_updates(param=layer.b,
                                                    m_param=self.m_db,
                                                    v_param=self.v_db,
                                                    lr=lr)

            # layer.Wex -= layer.Wex.grad * lr
            layer.Wex,self.m_dex,self.v_dex = self.compute_adam_updates(param=layer.Wex,
                                                m_param=self.m_dex,
                                                v_param=self.v_dex,
                                                lr=lr)
            
            # layer.Wei -= layer.Wei.grad * lr
            layer.Wei,self.m_dei,self.v_dei = self.compute_adam_updates(param=layer.Wei,
                                                m_param=self.m_dei,
                                                v_param=self.v_dei,
                                                lr=lr)
            # layer.Wix -= layer.Wix.grad * lr
            layer.Wix,self.m_dix,self.v_dix = self.compute_adam_updates(param=layer.Wix,
                                                m_param=self.m_dix,
                                                v_param=self.v_dix,
                                                lr=lr)
            if hasattr(layer, 'alpha'):
                # layer.alpha -= layer.alpha.grad *lr
                layer.alpha,self.m_dalpha,self.v_dalpha = self.compute_adam_updates(param=layer.alpha,
                                                        m_param=self.m_dalpha,
                                                        v_param=self.v_dalpha,
                                                        lr=lr)
...
class EiRNN_cSGD_UpdatePolicy(BaseUpdatePolicy, EiRNNcSGD_Mixin, EiRNN_UpdatePolicy):
...
class DalesANN_cSGD_UpdatePolicy(BaseUpdatePolicy, cSGD_Mixin, EiDense_UpdatePolicy):
    pass

class DalesANN_Adam_UpdatePolicy(BaseUpdatePolicy, cSGD_Mixin, EiDense_Adam_UpdatePolicy):
    pass

class DalesANN_Adam_UpdatePolicy(BaseUpdatePolicy, EiDense_Adam_UpdatePolicy):
    pass

class DalesANN_cSGD_Adam_UpdatePolicy(BaseUpdatePolicy, cSGD_Mixin, EiDense_Adam_UpdatePolicy):
    pass
